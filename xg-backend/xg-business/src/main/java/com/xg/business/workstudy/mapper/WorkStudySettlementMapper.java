...
public interface WorkStudySettlementMapper {
     ...
     * rate at settlement time — future rate changes on the position do not
     * retroactively reprice already-generated salary rows.
     *
     * 注：本结算路径假设 position.salary_unit='hour'（按小时×工时倍乘）。其他 unit
     * 走 employer 申报路径（WorkStudySalaryService.submit），不进此 SQL。
     *
    ...
    @Select("""
            SELECT t.id            AS timesheet_id,
                   ...
                   p.salary_amount AS hourly_rate
             ...
             WHERE t.status IN ('confirmed', 'finalized')
               ...
               AND COALESCE(p.salary_unit, 'hour') = 'hour'
               AND NOT EXISTS (

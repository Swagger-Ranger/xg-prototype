...
public class ApplicantClassMasterResolver implements RecipientTypeResolver {

    private final AssigneeLookupMapper assigneeLookup;

    @Override
    public String type() { return RecipientType.APPLICANT_CLASS_MASTER.code(); }
    ...
    public List<ResolvedRecipient> resolve(RecipientContext ctx, JsonNode spec) {
        ...
        return assigneeLookup.findClassMasterOfStudent(ctx.applicantId()).stream()
                .map(uid -> new ResolvedRecipient(uid, "class_master", cc))
                .toList();
    }
}

...
public class KnowledgeController {
    ...
    public R<KnowledgeQa> ask(
            @RequestBody @Validated AskQuestionRequest req,
            @RequestHeader("X-User-Id") Long userId) {
        return R.ok(knowledgeService.ask(req, userId));
    }

    @GetMapping("/api/v1/knowledge/qa-history")
    public R<PageResult<KnowledgeQa>> history(
            @RequestHeader("X-User-Id") Long userId,
            @Validated PageQuery query) {
        return R.ok(knowledgeService.history(userId, query));
    }
    ...
    public R<Void> feedback(
            ...
            @RequestHeader("X-User-Id") Long userId) {
        knowledgeService.feedback(id, req, userId);
        return R.ok();
    }
}

// Two-level administrative divisions: 省级 → 地级市. Municipalities (北京 /
// 上海 / 天津 / 重庆) are kept as a single child equal to the province name +
// "市" so the Cascader still renders two levels uniformly. Source: GB/T 2260
// trimmed to prefecture level. Matches the form_data convention of storing
// the leaf label (string) — never the code.
...
export const CHINA_PROVINCES: ProvinceNode[] = [
  { name: '北京', cities: ['北京市'] },
  { name: '天津', cities: ['天津市'] },
  ...
  { name: '台湾省', cities: ['台北市', '高雄市', '台中市', '台南市', '新北市', '基隆市', '新竹市', '嘉义市'] },
];

// antd Cascader expects { value, label, children? } recursively.
export const CASCADER_OPTIONS = CHINA_PROVINCES.map((p) => ({
  value: p.name,
  label: p.name,
  children: p.cities.map((c) => ({ value: c, label: c })),
}));

// Reverse map: leaf city name → [province, city]. Built once at import.
const CITY_PATH_MAP: Map<string, [string, string]> = (() => {
  const m = new Map<string, [string, string]>();
  for (const p of CHINA_PROVINCES) {
    for (const c of p.cities) m.set(c, [p.name, c]);
  }
  return m;
})();

/**
 * Best-effort match a free-text destination string to a [province, city]
 * Cascader path. Returns null when nothing reasonable matches — caller
 * should keep the original text and let the user edit.
 *
 * Strategy:
 *   1. Exact city name match ("南京市" → ["江苏省", "南京市"])
 *   2. Append "市" and retry ("南京"   → ["江苏省", "南京市"])
 *   3. Substring match on city ("北京市海淀区" → leaf contains "北京市")
 *   4. Province name match ("北京" → its first city for municipalities)
 */
export function matchCityPath(text: string): [string, string] | null {
  if (!text) return null;
  const trimmed = text.trim();
  if (!trimmed) return null;
  // 1
  const direct = CITY_PATH_MAP.get(trimmed);
  if (direct) return direct;
  // 2
  if (!trimmed.endsWith('市')) {
    const withCity = CITY_PATH_MAP.get(trimmed + '市');
    if (withCity) return withCity;
  }
  // 3: longest leaf that is a prefix of input
  let best: [string, string] | null = null;
  for (const [leaf, path] of CITY_PATH_MAP.entries()) {
    if (trimmed.startsWith(leaf) && (best === null || leaf.length > best[1].length)) {
      best = path;
    }
  }
  if (best) return best;
  // 4: province name match — only if province has exactly one city (municipalities)
  for (const p of CHINA_PROVINCES) {
    if ((p.name === trimmed || p.name === trimmed + '省' || p.name === trimmed + '市')
        && p.cities.length === 1) {
      return [p.name, p.cities[0]];
    }
  }
  return null;
}

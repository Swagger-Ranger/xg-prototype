...
- [x] 工作台首页可以展示 5 张任务卡（首屏不超过 5 个）
- [x] 任务卡的 6 个按钮（接单 / 完成 / 改期 / 转介 / 拒绝 / 详情）全部可用并写审计
- [x] 详情页 5 个区块顺序正确，触发证据和历史关怀默认折叠
- [x] 状态机非法迁移被后端拒绝并返回中文错误（通过 `BizException.message`，前端用 `describeApiError` 透传）
- [x] AI 区域三种状态（ready / pending / failed）UI 均正确显示，AI 不可用时其他按钮仍可点
- [x] 旧 `/alerts` 路由仍能访问，工作台首页底部有"切回旧视图"链接

**验收结论（2026-05-17 回填）**：6 项全部满足。证据锚点 —— 工作台首页 / 6 按钮 / 详情 5 区块在
W4-FE `6c4dd2d`；状态机非法迁移拒绝在 W2.4 `CareTaskTransitionsTest`（16 用例本地绿）+ W4 全栈 e2e
`429ce91`；AI 三态在 W3（`bc63cfd`→`db35aa1`）+ W4-FE `XiaoxiBrief`；"切回旧视图"toggle 在
`xg-frontend/apps/web/src/pages/care/index.tsx:140`（navigate('/alerts')）。

**后续周交付**：W3 AI brief / W4 通知闭环 / W5 管理视图与下钻 / W6 学生侧与规则运维 均已落地并经 docker
真实栈 e2e 验证，完成态登记见 `PRD-主动关怀工作台-重写版.md` §18.2。

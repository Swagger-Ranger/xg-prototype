...
```text
GET    /api/care/tasks?status=pending&limit=5&sort=severity_desc,due_asc
       → 工作台首页"今日待处理"

GET    /api/care/tasks?status=accepted,in_progress
GET    /api/care/tasks?status=resolved&since=monday
GET    /api/care/tasks?status=overdue

GET    /api/care/tasks/:taskId   → 详情
POST   /api/care/tasks/:taskId/accept
POST   /api/care/tasks/:taskId/resolve         body: {closed_reason, note}
POST   /api/care/tasks/:taskId/reject          body: {reason_code, reason_detail?}
POST   /api/care/tasks/:taskId/reschedule      body: {days: 1|3|7}
POST   /api/care/tasks/:taskId/transfer        body: {target_dept, reason_detail}

GET    /api/care/tasks/:taskId/brief           → 当前 AI brief（不存在则触发懒加载）
POST   /api/care/tasks/:taskId/brief/refresh   → 重新分析（5 分钟限流）
```

...
## 关于危机求助快速通道（不是第 14 条规则）

「危机求助快速通道」**不属于本 backlog 的 rule_id 体系，也不是 R013**。它是与 R001–R012 **并行**的 P1 例外通道：危机是单次即触发，与本文这套窗口阈值规则语义不同，**不进规则引擎、不占 rule_id**。它的范围 / 边界 / 状态见 `PRD-主动关怀工作台-重写版.md` §9.5 与 `危机求助快速通道-设计方案.md`。本 backlog 的 8/13 口径不受其影响。
